...
def main():
    ...
    #     "assignment_1/pingpong_results_reversed-order.csv"
    # )
    ...
    times_filtered[6] = np.mean(samples_per_element[2**6][1:]) * 1e6
    stds_filtered[6] = np.std(samples_per_element[2**6][1:]) * 1e6
    times_filtered[13] = np.mean(samples_per_element[2**13][1:]) * 1e6
    stds_filtered[13] = np.std(samples_per_element[2**13][1:]) * 1e6
    # times_filtered[16] = np.mean(samples_per_element[2**16][1:]) * 1e6
    # stds_filtered[16] = np.std(samples_per_element[2**16][1:]) * 1e6
    # Plot the results
    first_slope, first_intercept, _, _, _ = stats.linregress(
        no_elements[:9], times_filtered[:9]
    )
    second_slope, second_intercept, _, _, _ = stats.linregress(
        no_elements[9:15], times_filtered[9:15]
    )
    ...
    # Load two node case
    no_elements, times, stds, samples_per_element = read_data_from_csv(
        "assignment_1/pingpong_results_reversed-order_two-nodes.csv"
    )
    ...
    times_filtered[13] = np.mean(samples_per_element[2**13][1:]) * 1e6
    stds_filtered[13] = np.std(samples_per_element[2**13][1:]) * 1e6
    times_filtered[19] = np.mean(samples_per_element[2**19][1:]) * 1e6
    stds_filtered[19] = np.std(samples_per_element[2**19][1:]) * 1e6
    # Compute trend
    first_slope, first_intercept, _, _, _ = stats.linregress(
        no_elements[:9], times_filtered[:9]
    )
    ...
    fig.suptitle(r"Communication Delay Function ($2^{19} \rightarrow 2^{0}$) Reversed")
    ...
    plt.savefig("assignment_1/linear_trend_reversed.png")
    plt.show()
